...
async function submitDesign() {
  ...
  if (!user || !user.groupNo || !user.studentNo) {
    ElMessage.error('未获取到学生身份，无法提交设计')
    return
  }
  ...
  try {
    // 提交到教师端
    const ack = await socketService.submit(payload)
    ...
    // 保存到本地存储
    saveToLocalStorage()
    console.log('[Activity7] 问题设计已保存到本地存储')
...
const getStorageKey = () => {
  ...
  return `activity7_${user.groupNo}_${user.studentNo}`
}
