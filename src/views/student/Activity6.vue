...
const loadViewpointFromActivity5 = () => {
  ...
  const activity5Key = `activity5_vote_${g}_${s}`
  try {
...
const submitEditedData = async () => {
  ...
  try {
    ...
    const teacherAck = await socketService.submit(teacherPayload as any)
    if (teacherAck.code !== 200) {
      throw new Error(teacherAck.message || '发送给教师失败')
    }
    
    // 发送给其他学生 (discuss)
    const discussPayload = {
      type: 'activity6_discuss',
      from: { groupNo: g, studentNo: s },
      data: { 
        q1: analysisData.value.q1.trim(),
        q2: analysisData.value.q2.trim(),
    ...
    const discussAck = await socketService.discuss(discussPayload as any)
    if (discussAck.code !== 200) {
      console.warn('[Activity6] 发送discuss失败:', discussAck.message)
    }
...
const getStorageKey = () => {
  const g = groupNo.value
  const s = studentNo.value
  return g && s ? `activity6_data_${g}_${s}` : null
}
